...
function TestComponent() {
  ...
  return (
    <div>
      <button
        onClick={() =>
          dispatch({ type: "START_PROCESS", payload: { text: "hola mundo" } })
        }
      >
        start
      </button>
...
describe("ProcessContext persistence", () => {
  ...
  beforeEach(() => {
    ...
    const setItemImpl = (k: string, v: string) => {
      store[k] = v;
    };
    ...
    // attach spy for assertions on global (test-only helper)
    (global as unknown as { __setItemSpy?: typeof setItemSpy }).__setItemSpy =
      setItemSpy;
  });
  ...
  it("persists state after starting process", async () => {
    ...
    const spy = (
      global as unknown as { __setItemSpy?: ReturnType<typeof vi.fn> }
    ).__setItemSpy as ReturnType<typeof vi.fn> | undefined;
    expect(spy).toBeDefined();
    if (!spy) {
      throw new Error("setItem spy was not initialized");
    }
    // find any call where payload includes our new input
    const calls = (spy.mock.calls as [string, string][]).map(([key, val]) => ({
      key,
      value: JSON.parse(val),
    }));
    const match = calls.find(
      (c) =>
        c.key === "exploramodelo_state" &&
        c.value.inputText === "hola mundo" &&
        c.value.currentStep === 1,
    );
