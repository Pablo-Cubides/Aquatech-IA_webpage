...
const MenuCasos = () => {
  ...
  const casosFiltrados = useMemo(() => {
    return (Array.isArray(casos) ? casos : []).filter(caso =>
      ...
      ((caso.razon_filtro || '') as string).toLowerCase().includes(busqueda.toLowerCase())
    );
  }, [casos, busqueda]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        const next = selectedIndex >= 0 ? Math.min(selectedIndex + 1, casos.length - 1) : 0;
        selectCaso(next);
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        const prev = selectedIndex > 0 ? selectedIndex - 1 : Math.max(casos.length - 1, 0);
        selectCaso(prev);
      }
    };
