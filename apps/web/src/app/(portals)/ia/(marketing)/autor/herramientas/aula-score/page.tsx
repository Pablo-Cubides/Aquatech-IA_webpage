...
export default function ConfigurationPage() {
  ...
  const [groupNames, setGroupNames] = useState<string[]>(['Grupo 1', 'Grupo 2', 'Grupo 3']);
  ...
  const handleNumGroupsChange = (num: number) => {
    ...
    setGroupNames(Array(clamped).fill('').map((_, i) => `Grupo ${i + 1}`));
    setError('');
  };
  ...
  const handleContinue = () => {
    ...
    sessionStorage.setItem(
      'aula-score-groups',
      JSON.stringify(groupNames.map((name) => ({ id: crypto.randomUUID(), name, score: 0 })))
    );
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="text-foreground-secondary">Aventuras de juego en clase</p>
        </div>
        ...
        <div className="card space-y-6">
          {step === 1 ? (
            <>
              <div>
                <label className="block heading-tertiary mb-4">¿Cuántos grupos van a participar?</label>
                <input
                  type="number"
                  min="1"
                  max="30"
                ...
                />
                <p className="text-foreground-secondary text-sm mt-2">Mínimo 1, máximo 30</p>
              </div>
              {error && <div className="text-danger-color font-medium">{error}</div>}
            ...
            <>
              <div>
                <label className="block heading-tertiary mb-4">Nombres de los grupos</label>
                <p className="text-foreground-secondary text-sm mb-4">Ingresa un nombre único para cada grupo</p>

                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {groupNames.map((name, index) => (
                    <div key={index}>
                      <label className="block text-sm font-medium mb-1">Grupo {index + 1}</label>
                      <input
                        type="text"
                        value={name}
