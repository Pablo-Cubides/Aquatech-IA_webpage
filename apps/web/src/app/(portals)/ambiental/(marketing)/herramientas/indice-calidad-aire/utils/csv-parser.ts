...
function normalizeColumnName(column: string): PollutantId | null {
  const normalized = column.toLowerCase().trim().replace(/[^a-z0-9._ ]/g, '');
...
function parseValue(value: string): number | null {
  if (!value || value.trim() === '' || value === 'N/A' || value === 'null' || value === '-') {
    return null;
  }
...
export function csvToMeasurements(text: string): CSVParseResult {
  ...
  if (rows.length < 2) {
    result.errors.push('El archivo CSV debe tener al menos un encabezado y una fila de datos');
    return result;
  }
  ...
  const hasParameterColumn = headers.includes('parametro') || headers.includes('parameter');
...
function parseWideFormat(
  ...
  const dateCol = headers.findIndex(h => 
    h.includes('fecha') || h.includes('date') || h.includes('datetime')
  );
  const stationCol = headers.findIndex(h => 
    h.includes('estacion') || h.includes('station') || h.includes('ubicacion') || h.includes('location')
  );
  
  if (dateCol === -1) {
    result.warnings.push('No se encontró columna de fecha, usando fecha actual');
  }
  ...
  if (pollutantCols.length === 0) {
    result.errors.push('No se encontraron columnas de contaminantes (pm25, pm10, o3, no2, so2, co)');
    return result;
  }
...
function parseLongFormat(
  ...
  const stationCol = headers.findIndex(h => 
    h.includes('estacion') || h.includes('station') || h.includes('ubicacion')
  );
  const paramCol = headers.findIndex(h => 
    h.includes('parametro') || h.includes('parameter')
  );
  ...
  const unitCol = headers.findIndex(h => 
    h.includes('unidad') || h.includes('unit')
  );
  
  if (paramCol === -1 || valueCol === -1) {
    result.errors.push('Formato largo requiere columnas: parametro/parameter, valor/value');
    return result;
  }
