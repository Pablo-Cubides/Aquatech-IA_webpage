...
export default function ComparisonResults({
  ...
  const indicatorGroups: Map<
    string,
    { country: string; data: number[]; fullName: string }[]
  > = new Map();
  ...
  results.countries.forEach((fullName, idx) => {
    ...
    const group = indicatorGroups.get(indicator);
    if (!group) {
      return;
    }
    group.push({
  ...
  return (
    <div className="space-y-8">
      ...
      {Array.from(indicatorGroups.entries()).map(([indicator, countries]) => {
