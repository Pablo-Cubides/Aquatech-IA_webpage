...
const SECTOR_ICON_PATTERNS = [
  ...
  },
  {
    keywords: [
      "vida_acuatica",
      "proteccion_vida_acuatica",
      "conservacion_ambiente_acuatico",
    ],
    icon: <span className="text-2xl">🐟</span>,
  },
  {
...
function ExplorarContent() {
   ...
   */
  const getNormativeUrl = (
    normativeRef: string,
    dataContext?: AnyRecord,
  ): string | null => {
    ...
    const allSectors =
      (dataToUse as AnyRecord)?._sectors || (dataToUse as AnyRecord)?.sectors;
    ...
    if (allSectors && typeof allSectors === "object") {
      for (const sectorKey in allSectors) {
        ...
        if (sectorObj) {
          const sectorNormRef =
            sectorObj.normativeReference ?? sectorObj.normativeReference_es;
          const directUrl =
            sectorObj.normativeUrl ?? sectorObj.normativeReference_url;

          if (sectorNormRef === normativeRef && directUrl) {
    ...
    if (dataToUse?.sources && Array.isArray(dataToUse.sources)) {
      ...
      for (const source of dataToUse.sources) {
        ...
        if (
          normalize(sourceName) === normalizedRef ||
    ...
    if (selectedCountry && selectedDomain) {
      const countrySources =
        REGULATORY_SOURCES[selectedCountry]?.[selectedDomain];
      if (
        countrySources &&
        Array.isArray(countrySources) &&
  ...
  const renderNormativeReference = (
    normRef: string,
    data?: AnyRecord,
  ): React.ReactNode => {
      ...
      <div className="relative overflow-hidden border-b shadow-lg border-slate-200/50 bg-gradient-to-r from-slate-900 via-blue-900 to-emerald-900">
        ...
        <div className="container relative px-4 py-6 mx-auto">
          ...
          <p className="text-lg text-blue-100">
            Consulta estándares ambientales internacionales
          </p>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <Card className="mb-8 border-0 shadow-lg bg-white/80 backdrop-blur-sm">
            <CardContent className="p-6">
              ...
              <div className="mb-8">
                <label className="block mb-4 text-sm font-semibold tracking-wide text-gray-900 uppercase">
                  Dominio Ambiental
                </label>
                <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                  {DOMINIOS.map((dominio) => (
                    ...
                    >
                      ...
                      <div
                        className={`text-sm font-semibold line-clamp-2 ${selectedDomain === dominio.id ? "text-white" : "text-gray-900"}`}
                      >
                        {dominio.label}
                      </div>
                    </button>
              ...
              <div
                className={`grid gap-6 ${(availableSectors?.length || 0) > 1 ? "md:grid-cols-3" : "md:grid-cols-2"}`}
              >
            ...
            <>
                      ...
                      <div className="flex items-center gap-3 text-blue-100">
                        ...
                        {filteredRecords.length > 0 && (
                          <>
                            <span>•</span>
                            <span>
                              {filteredRecords.length} parámetro
              ...
              <div className="space-y-6">
                {Object.entries(sectionsToDisplay).length > 0 ? (
                  Object.entries(sectionsToDisplay).map(
                    ([sectionKey, records], index) => {
                      // Use smart icon matching instead of direct lookup
                      const sectionIcon = getSectorIcon(
                        sectionKey,
                        currentDominio.icon,
                      );
                      // Prefer a human-friendly sector name when available on records
                      const sectionLabel = (() => {
                        // records is the array for this section (shadowed in map)
                        const firstRec =
                          records && records.length > 0
                            ? (records[0] as AnyRecord)
                            : null;
                      ...
                      return (
                        <Card
                          key={index}
                          className="overflow-hidden border-0 shadow-lg print-friendly"
                        >
                          <CardHeader className="pb-4 border-b bg-gradient-to-r from-slate-100 to-slate-50 border-slate-200">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <CardTitle className="text-xl text-gray-900 capitalize line-clamp-2">
                                    {sectionLabel}
                                  </CardTitle>
                                ...
                                <div>
                                  <div className="text-sm font-semibold text-amber-900">
                                    {(() => {
                                      ...
                                      const allSectors =
                                        (data as AnyRecord)?._sectors ||
                                        (data as AnyRecord)?.sectors;
                                      // Try to find the sector object, with flexible key matching
                                      let sectorObj: AnyRecord | undefined =
                                        undefined;

                                      if (
                                        allSectors &&
                                        typeof allSectors === "object"
                                      ) {
                                        // First try direct match
                                        sectorObj = (
                                          allSectors as Record<
                                            string,
                                            AnyRecord
                                          >
                                        )[sectionKey];

                                        // If not found, try normalizing both sides (underscores vs hyphens)
                                        if (!sectorObj) {
                                          const normalizedKey =
                                            sectionKey.replace(/-/g, "_");
                                          sectorObj = (
                                            allSectors as Record<
                                              string,
                                              AnyRecord
                                      ...
                                      if (
                                        ...
                                        sectorObj?.normativeSources.length > 0
                                      ) {
                                        return (
                                          <div className="flex flex-col gap-1">
                                            {sectorObj.normativeSources.map(
                                              (s: AnyRecord, idx: number) => (
                                                <a
                                                  key={idx}
                                                  href={String(s.url ?? "")}
                                                  target="_blank"
                                      ...
                                      const generalSources = (data as AnyRecord)
                                        ?.normativeSources;
                                      if (
                                        Array.isArray(generalSources) &&
                                      ...
                                      const directUrl =
                                        ...
                                        sectorObj?.normativeReference_url;
                                      if (directUrl) {
                                        const label = String(
                                          sectorObj?.normativeSourceName ??
                                            sectorObj?.normativeReference ??
                                            sectorObj?.normativeReference_es ??
                                            sectorObj?.name ??
                                            "Fuente normativa",
                                        );
                                        return (
                                      ...
                                      if (
                                        !sectorObj?.normativeSources &&
                                        typeof textToUse === "string" &&
                                        /,|;| y | and /i.test(textToUse)
                                      ) {
                                        const parts = String(textToUse)
                                          .split(/,|;| y | and /i)
                                          .map((p) => p.trim())
                                          .filter(Boolean);
                                        return (
                                      ...
                                      return renderNormativeReference(
                                        textToUse as string,
                                        data,
                                      );
                                    })()}
                            ...
                            <div className="overflow-x-auto">
                              <table className="w-full text-sm">
                                <thead className="border-b bg-slate-50 border-slate-200">
                                  <tr>
                                    <th className="px-4 py-3 text-xs font-semibold tracking-wide text-left text-gray-900 uppercase">
                                      Parámetro
                                    </th>
                                    <th className="px-4 py-3 text-xs font-semibold tracking-wide text-left text-gray-900 uppercase">
                                      Valor Límite
                  ...
                  <Card className="border-0 shadow-lg bg-gradient-to-br from-slate-50 to-blue-50">
                    <CardContent className="p-12 text-center">
                      ...
                      <p className="max-w-md mx-auto text-gray-600">
                        Intenta cambiar los filtros, dominio o el término de
                        búsqueda para encontrar los estándares que buscas.
                      </p>
                    </CardContent>
                  </Card>
                )}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-amber-800">
                        Esta información se basa en normatividad disponible.
                        Siempre verifica la vigencia actual consultando los
                        diarios oficiales o autoridades competentes de cada
                        país, ya que las regulaciones pueden cambiar.
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-50 via-white to-emerald-50">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="max-w-md mx-auto mb-6 text-gray-600">
                    Selecciona un dominio ambiental, elige un país
                    {(availableSectors?.length || 0) > 1
                      ? " y un sector "
                      : " "}
                    y explora los estándares vigentes.
                  </p>
