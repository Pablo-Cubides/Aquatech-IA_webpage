...
export function normalizeResponseFormat(
  ...
  if (
    (domain === "agua" || domain === "vertimientos") &&
    data.sectors &&
    ...
    for (const [sectorId, sectorData] of Object.entries(data.sectors || {})) {
      ...
      if (activeSector !== null) {
        ...
        if (sectorId === activeSector) {
          ...
          if (
            sector["normativeReference"] &&
            typeof sector["normativeReference"] === "string"
          ) {
    ...
    const countryDomainSources: RegulatorySource[] =
      REGULATORY_SOURCES[countryKey] && REGULATORY_SOURCES[countryKey][domain]
        ? REGULATORY_SOURCES[countryKey][domain]
        : [];
    result.sources = countryDomainSources;
    if (
      Array.isArray(result.sources) &&
      (result.sources as RegulatorySource[]).length > 0
    ) {
    ...
    try {
      ...
      for (const [_sId, sData] of Object.entries(result._sectors || {})) {
        ...
        if (s["normativeReference"])
          candidates.push(String(s["normativeReference"]));
        if (s["normativeReference_es"])
          candidates.push(String(s["normativeReference_es"]));
        ...
        for (const cand of candidates) {
          ...
          const codeMatches =
            String(cand).match(/\bGB\s*\d{2,6}(?:-\d{2,6})?\b/gi) || [];
          const normalizedCodes = codeMatches.map((c: string) =>
            normalizeText(c.replace(/\s+/g, " ")),
          );
          for (const src of countryDomainSources) {
            ...
            if (
              (name && name.includes(candNorm)) ||
              (desc && desc.includes(candNorm)) ||
              (candNorm.includes(name) && name.length > 3) ||
              (candNorm.includes(desc) && desc.length > 3)
            ) {
              // avoid duplicates
            ...
            for (const code of normalizedCodes) {
              if (
                (name && name.includes(code)) ||
                (desc && desc.includes(code))
              ) {
                if (!foundMatches.find((f) => f.url === src.url)) {
                  foundMatches.push({
                    name: src.name,
          ...
          for (const p of parts) {
            ...
            for (const src of countryDomainSources) {
              ...
              const desc = normalizeText(src.description);
              if (
                (name && name.includes(pNorm)) ||
                (desc && desc.includes(pNorm)) ||
                (pNorm.includes(name) && name.length > 3)
              ) {
                if (!foundMatches.find((f) => f.url === src.url)) {
                  foundMatches.push({
                    name: src.name,
    ...
    if (sectorNormativeReference) {
      ...
      const sourcesArray = Array.isArray(result.sources)
        ? (result.sources as RegulatorySource[])
        : [];
      const topFound = sourcesArray.find((src) =>
        (src.name || "")
          .toString()
          .toLowerCase()
          .includes(String(sectorNormativeReference).toLowerCase()),
      );
      if (topFound)
        (result as AnyRecord)["normativeReference_url"] = topFound.url;
    }
    ...
    try {
      const topRef =
        result.normativeReference_es || result.normativeReference || "";
      if (
        topRef &&
        Array.isArray(result.sources) &&
        (result.sources as unknown[]).length > 0
      ) {
        ...
        for (const src of result.sources as RegulatorySource[]) {
          ...
          if (
            potableKeys.some(
              (k) =>
                sName.includes(k) || sDesc.includes(k) || topLower.includes(k),
            )
          ) {
            targets.push("agua_potable");
          }
          if (
            superficialKeys.some(
              (k) =>
                sName.includes(k) || sDesc.includes(k) || topLower.includes(k),
            )
          ) {
            // map to any sector that looks like 'aguas_superficiales' or contains 'superficiales'
            const superficialSectorKeys = Object.keys(
              result._sectors || {},
            ).filter((k) =>
              /superficial|superficiales|superficie|clase/i.test(k),
            );
            if (superficialSectorKeys.length > 0)
              targets.push(...superficialSectorKeys);
          }
          ...
          for (const t of new Set(targets)) {
            const sectors = (result._sectors || {}) as Record<
              string,
              AnyRecord
            >;
            ...
            if (
              !(sectorObj["normativeSources"] as MatchedSource[]).find(
                (f) => f.url === src.url,
              )
            ) {
              (sectorObj["normativeSources"] as MatchedSource[]).push({
                name: src.name,
                url: src.url,
                description: src.description,
              });
    ...
    try {
      ...
      if (topRef) {
        ...
        const codes = Array.from(
          new Set(
            (String(topRef).match(/\bgb\s*\d{2,6}(?:-\d{2,6})?\b/gi) || []).map(
          ...
          for (const code of codes) {
            ...
            const matchedSources = sourcesForCodes.filter((s) => {
              ...
              return (
                name.includes(code) ||
                desc.includes(code) ||
                name.includes(code.replace(/gb/, "").trim())
              );
            });
            ...
            if (code.includes("5749")) {
              ...
              if (sectorObj) {
                ...
                if (!sectorObj.normativeUrl) {
                  sectorObj["normativeUrl"] = (
                    matchedSources as RegulatorySource[]
                  )[0].url;
                  sectorObj["normativeSourceName"] = (
                    matchedSources as RegulatorySource[]
                  )[0].name;
                }
              }
            ...
            if (code.includes("3838")) {
              ...
              const sectors = (result._sectors || {}) as Record<
                ...
                AnyRecord
              >;
              const superficialKeys = Object.keys(sectors).filter((k) =>
                /superficial|superficiales|superficie|clase/i.test(k),
              );
    ...
    result.records = (data.records as AnyRecord[]).filter(
      (r) => r["_sector"] === sectorFilter || r["sector"] === sectorFilter,
    );
...
export async function GET(request: NextRequest) {
  ...
  if (!country) {
    return NextResponse.json(
      { error: "País no válido o no soportado" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      return NextResponse.json(
        { error: `País no encontrado en el dominio "${domain}"` },
        { status: 404 },
      );
    }
    ...
    try {
      ...
      const normalizedForFrontend = normalizeResponseFormat(
        validatedData,
        domain,
        country,
        sector || undefined,
      ...
      logger.warn("normas:validation_failed", {
        domain,
        country,
        issues: zerr.issues,
      });
      ...
      try {
        ...
        logger.warn("normas:failed_write_validation_log", {
          domain,
          country,
          error: String(logErr),
        });
      ...
      try {
        ...
        type BestEffort = Partial<typeof mergedNormalized> & {
          records?: Record<string, unknown>[];
          registros?: Record<string, unknown>[];
        };
        const bestEffort: BestEffort & Record<string, unknown> = {
          ...(mergedNormalized as object),
        } as BestEffort & Record<string, unknown>;

        const getStr = (v: unknown) => (typeof v === "string" ? v : "");

        bestEffort.country =
          getStr(bestEffort.country) || getStr(bestEffort["pais"]) || country;
        bestEffort.version =
          getStr(bestEffort.version) ||
          getStr(bestEffort["lastUpdate"]) ||
          getStr(bestEffort["version"]) ||
          "";
        ...
        if (Array.isArray(bestEffort.records)) {
          bestEffort.records = bestEffort.records.map((r: unknown) => {
            ...
            const parameter =
              getStr(rec["parameter"]) || getStr(rec["parametro"]) || "";
            const limit =
              getStr(rec["limit"]) ||
              getStr(rec["limite"]) ||
              getStr(rec["lim"]) ||
            ...
            const unit =
              typeof rec["unit"] === "string"
                ? String(rec["unit"])
                : typeof rec["unidad"] === "string"
                  ? String(rec["unidad"])
                  : null;
            const notes = Array.isArray(rec["notes"])
              ? (rec["notes"] as unknown[]).map(String)
              : Array.isArray(rec["notas"])
                ? (rec["notas"] as unknown[]).map(String)
                : [];
            ...
            if (refRaw) {
              if (typeof refRaw === "object" && refRaw !== null) {
                ...
                reference = {
                  standard: getStr(rObj["standard"]) || getStr(rObj["norma"]),
                };
              } else {
            ...
            const category =
              getStr(rec["category"]) || getStr(rec["categoria"]) || undefined;
        ...
        if (bestEffort.records && !bestEffort.registros) {
          bestEffort.registros = bestEffort.records.map((r: unknown) => {
            ...
            return {
              ...
              notas: Array.isArray(rr["notes"])
                ? (rr["notes"] as unknown[]).map(String)
                : undefined,
            };
          });
        }
        ...
        if (bestEffort.registros && !bestEffort.records) {
          bestEffort.records = bestEffort.registros.map((r: unknown) => {
            const rec = (r ?? {}) as Record<string, unknown>;
            return {
              parameter: rec["parametro"]
                ? String(rec["parametro"])
                : rec["parameter"]
                  ? String(rec["parameter"])
                  : "",
              limit: rec["limite"]
                ? String(rec["limite"])
                : rec["limit"]
        ...
        if (ref && !bestEffort["referencia"]) {
          if (typeof ref === "object" && ref !== null) {
            ...
            bestEffort["referencia"] = {
              norma: getStr(rObj["standard"]) || getStr(rObj["norma"]),
            };
          }
        }
        if (bestEffort["referencia"] && !bestEffort["reference"]) {
          ...
          if (typeof r === "object" && r !== null) {
            ...
            bestEffort["reference"] = {
              standard: getStr(rObj["norma"]) || getStr(rObj["standard"]),
            };
          } else {
        ...
        const result: Record<string, unknown> = {
          ...(bestEffort as object),
          _validation: zerr.flatten(),
        };
        ...
        logger.error("normas:coerce_failed", {
          domain,
          country,
          error: String(e),
        });
        return NextResponse.json(
          {
            error:
              "La estructura del archivo de datos está corrupta o no es válida y no fue posible normalizarla automáticamente.",
    ...
    logger.error("normas:file_error", {
      domain,
      country,
