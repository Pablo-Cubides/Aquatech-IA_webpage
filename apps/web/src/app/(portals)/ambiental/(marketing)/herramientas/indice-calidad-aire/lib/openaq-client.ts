...
export async function getMeasurements(params: {
  ...
  try {
    ...
    for (const m of data.results || []) {
      ...
      grouped.get(datetime)!.push({
        pollutantId,
        value: m.value,
        unit: m.unit === 'µg/m³' || m.unit === 'ug/m3' ? 'µg/m³' : 
              m.unit === 'mg/m³' || m.unit === 'mg/m3' ? 'mg/m³' : 'µg/m³',
      });
    }
 ...
 */
export async function getLatestByCountry(countryCode: string, limit = 50): Promise<AirQualityMeasurement[]> {
  try {
    const response = await fetch(
    ...
    return (data.results || []).map((loc: {
      ...
      return {
        ...
        location: `${loc.city || ''}, ${loc.country || ''}`.replace(/^, |, $/g, ''),
 ...
 */
export async function getCountries(): Promise<Array<{ code: string; name: string; count: number }>> {
  try {
    const response = await fetch(
      `${OPENAQ_BASE_URL}/countries?limit=200`,
      {
        headers: {
          'Accept': 'application/json',
        },
    ...
    return (data.results || []).map((c: { code: string; name: string; locations: number }) => ({
      code: c.code,
      name: c.name,
      count: c.locations || 0,
    }));
