...
export async function exportCorrelationsPDF(
  ...
  const useLandscape = data.numericColumns.length > 3 || data.type !== 'correlation';
  ...
  doc.setFontSize(16);
  doc.setTextColor(31, 41, 55);
  const titleText = data.type === 'growth' ? "Reporte de Análisis de Crecimiento" : 
                    data.type === 'trend' ? "Reporte de Análisis de Tendencias" : 
                    data.type === 'comparison' ? "Reporte de Análisis Comparativo" :
  ...
  doc.text(`Generado el: ${new Date().toLocaleString()}`, pageWidth / 2, yPosition, { align: "center" });
  yPosition += 4;
  doc.text(`Fuente: ${data.filename}`, pageWidth / 2, yPosition, { align: "center" });
  yPosition += 6;
  ...
  if (data.type === 'growth' || data.type === 'trend' || data.type === 'comparison') {
      if (contentRef) {
          doc.setFontSize(12);
          doc.setTextColor(31, 41, 55);
          ...
          for (const child of children) {
              try {
                  const dataUrl = await toPng(child, { backgroundColor: '#ffffff', quality: 0.95 });
                  ...
                  doc.addImage(dataUrl, "PNG", margin, yPosition, pdfImgWidth, pdfImgHeight);
      ...
      doc.text(`• Variables analizadas: ${data.numericColumns.length}`, margin + 5, yPosition);
      yPosition += 5;
      doc.text(`• Registros procesados: ${data.rawDataCount}`, margin + 5, yPosition);
      yPosition += 5;
      doc.text(`• Pares correlacionados: ${data.correlationResults.length}`, margin + 5, yPosition);
      yPosition += 10;
      ...
      if (contentRef) {
        ...
        try {
          const dataUrl = await toPng(contentRef, { backgroundColor: '#ffffff', quality: 0.95 });
          ...
          doc.addImage(dataUrl, "PNG", (pageWidth - pdfImgWidth) / 2, yPosition, pdfImgWidth, pdfImgHeight);
      ...
      // Correlation Table
      const sorted = [...data.correlationResults].sort((a, b) => {
      ...
      const colWidths = useLandscape ? [80, 80, 30, 30, 30] : [50, 50, 25, 25, 20];
      const startX = margin;
      ...
      yPosition += 4;
      doc.setFont("helvetica", "normal");
      const pearsonText = "Mide la relación lineal entre dos variables. Valores cercanos a +1 indican correlación positiva fuerte, valores cercanos a -1 indican correlación negativa fuerte, y valores cercanos a 0 indican que no existe relación lineal.";
      const pearsonLines = doc.splitTextToSize(pearsonText, pageWidth - margin * 2);
      doc.text(pearsonLines, margin, yPosition);
      yPosition += pearsonLines.length * 4 + 3;
      
      // Interpretación
      doc.setFont("helvetica", "italic");
      doc.text("Interpretación: 0.75-1.00 = Muy fuerte | 0.50-0.74 = Moderada | 0.25-0.49 = Débil | 0.00-0.24 = Sin correlación", margin, yPosition);
      yPosition += 6;
      doc.setFont("helvetica", "normal");
      ...
      yPosition += 4;
      doc.setFont("helvetica", "normal");
      const spearmanText = "Evalúa la relación monotónica entre variables usando el orden de los datos en lugar de valores exactos. Es más robusta ante valores atípicos (outliers) y es ideal cuando los datos no siguen una distribución normal.";
      const spearmanLines = doc.splitTextToSize(spearmanText, pageWidth - margin * 2);
      ...
      yPosition += 4;
      doc.setFont("helvetica", "normal");
      const kendallText = "Mide la concordancia entre pares de observaciones. Es la más conservadora y funciona bien con muestras pequeñas o cuando hay muchos empates en los datos.";
      const kendallLines = doc.splitTextToSize(kendallText, pageWidth - margin * 2);
      ...
      doc.setFont("helvetica", "bold");
      doc.text("💡 Recuerda: Correlación no implica causalidad.", margin + 3, yPosition + 5);
      doc.setFont("helvetica", "normal");
      doc.text("Que dos variables estén correlacionadas no significa que una cause la otra.", margin + 3, yPosition + 9);
  }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text("Generado por Aquatech IA | www.aquatechia.com", margin, pageHeight - 8);
  }

  doc.save(`Reporte_${data.type === 'growth' ? 'Crecimiento' : 'Correlaciones'}_${data.filename.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`);
}
