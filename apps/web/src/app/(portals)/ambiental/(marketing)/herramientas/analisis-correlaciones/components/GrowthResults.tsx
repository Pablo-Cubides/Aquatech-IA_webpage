    ...
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-sm text-blue-600 mt-2">
          ✓ Se procesaron {indicators.length} indicador(es) con datos
          disponibles
        </p>
        {indicators.length > 1 && (
          <p className="text-xs text-gray-400 mt-1">
            ↓ Desplázate hacia abajo para ver todos los gráficos (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="p-4 bg-blue-50 rounded-lg">
                ...
                <span
                  className={`text-2xl font-bold ${data.cagr >= 0 ? "text-blue-800" : "text-red-600"}`}
                >
                  {data.cagr >= 0 ? "+" : ""}
                  {data.cagr.toFixed(2)}%
                </span>
              ...
              <div className="p-4 bg-green-50 rounded-lg">
                <span className="block text-sm text-green-600 mb-1">
                  Cambio Total ({data.periods[0]} -{" "}
                  {data.periods[data.periods.length - 1]})
                </span>
                <span
                  className={`text-2xl font-bold ${data.totalChange >= 0 ? "text-green-800" : "text-red-600"}`}
                >
                  {data.totalChange >= 0 ? "+" : ""}
              ...
              <div className="p-4 bg-purple-50 rounded-lg">
                ...
                <span
                  className={`text-2xl font-bold ${data.avgGrowthRate >= 0 ? "text-purple-800" : "text-red-600"}`}
                >
                  {data.avgGrowthRate >= 0 ? "+" : ""}
                  {data.avgGrowthRate.toFixed(2)}%
                </span>
              </div>
            </div>
            ...
            <div className="h-[300px] w-full mb-4">
              <h4 className="text-sm font-medium text-gray-500 mb-2">
                Evolución en el Tiempo (pasa el cursor para ver % cambio anual)
              </h4>
