import { Calendar, Plus, Clock, Image as ImageIcon } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Programador de Contenido</h1>
          <p className="text-slate-400 mt-2">Gestiona y programa las publicaciones de la semana.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
        {WEEK_DAYS.map((day, index) => (
          <div key={day.name} className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col h-[600px]">
            <div className={`p-3 border-b border-slate-800 text-center ${
              index === 0 ? 'bg-emerald-900/20 text-emerald-400' : 'text-slate-400'
            }`}>
              ...
              {index === 0 && (
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-3 cursor-pointer hover:border-blue-500 transition-colors">
                  ...
                  <div className="mt-2 flex items-center justify-between text-xs text-slate-500">
                    ...
                    <span className="bg-yellow-500/10 text-yellow-500 px-1.5 py-0.5 rounded">Pendiente</span>
                  </div>
                </div>
              )}
            </div>
