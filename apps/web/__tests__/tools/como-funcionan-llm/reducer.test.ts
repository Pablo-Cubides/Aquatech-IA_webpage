...
describe("appReducer COMPUTE_EMBEDDINGS", () => {
  it("computes embeddings and positional encodings and stores them", () => {
    ...
    const action: Action = {
      ...
      payload: { tokenIds, tokens },
    } as unknown as Action;
    const next = appReducer(
      startState as unknown as import("@/tools/llm/context/ProcessContext").AppState,
      action,
    );
...
describe("appReducer COMPUTE_ATTENTION and PROBABILITIES", () => {
  it("computes attention heads and combined weights", () => {
    ...
    const afterAttention = appReducer(
      startState as unknown as import("@/tools/llm/context/ProcessContext").AppState,
      { type: "COMPUTE_ATTENTION" } as Action,
    );
    expect(afterAttention.processData?.attentionHeads.length).toBeGreaterThan(
      0,
    );
    expect(afterAttention.processData?.attentionWeights.length).toBeGreaterThan(
      0,
    );
  });
  ...
  it("computes probabilities and vocab when absent", () => {
    ...
    const afterProbs = appReducer(
      startState as unknown as import("@/tools/llm/context/ProcessContext").AppState,
      { type: "COMPUTE_PROBABILITIES" } as Action,
    );
